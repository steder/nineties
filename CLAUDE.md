...
## Conventions for typed Django code

Pyright + django-stubs gives us static analysis but does NOT infer the
auto-generated `<fk>_id` attribute that Django produces for `ForeignKey`
fields. When you write a model with an FK and reference `obj.kid_id` in
code, add an explicit class-level annotation:

```python
class Loan(models.Model):
    kid = models.ForeignKey("kids.Kid", on_delete=models.CASCADE)
    kid_id: int  # auto-generated by Django's FK descriptor; declared for pyright
```

This is purely a type-checker hint — runtime behavior is unchanged — but it
keeps `just typecheck` clean and matches what we'd write with django-types
later if we migrate.
